...
class OracleGenerator extends SQLGenerator {
  ...
  protected def parseCreate(create: CreateView, schemaMap: Map[String, AbstractSchema]): String = {
    ...
    System.out.println("parseUpsertMeta1111="+ddl)
  ...
  protected def parseUpsertMeta(q: UpsertRecord): String = {
    ...
    records.foreach {
      record =>
        ...
        queryResult += (s"('${name}','${schema}','${dataInterval}','${stats}',to_date('${createTime}','yyyy-MM-dd'))")
    }
    s"""
       |insert into ${q.dataset} (name, schema, dataInterval, stats, createTime) values
