...
class OracleGeneratortest extends Specification {
  ...
  val dataSetForSQL = OracleEarthquakeDataStore.DatasetName
  val schemaForSQL = OracleEarthquakeDataStore.EarthquakeSchema

  val schemaMapForSQL = Map(dataSetForSQL -> schemaForSQL)
  val startTime = "2016-01-01"
  val endTime = "2016-12-01"
  val startDate = StringField(OracleEarthquakeDataStore.TimeFieldName)
  val observation = StringField("OBSVALUE")
  val timeFilter = FilterStatement(startDate, None, Relation.inRange, Seq(startTime, endTime))

  val selectvalue = SelectStatement(Seq.empty, Seq.empty, 0, 0, Seq(observation))
  ...
  "OracleGenerator generate" should {
    ...
    "translate a simple query group by hour" in {
      ...
      val query = new Query(dataSetForSQL, Seq.empty, Seq.empty, filter, Seq.empty,None)
      val result = parser.generate(query, schemaMapForSQL)
      removeEmptyLine(result) must_== unifyNewLine(
        """|select t."OBSVALUE" as "OBSVALUE"
           |from "Qz_9130" t
           |where t."STARTDATE" >= '2016-01-01' and t."STARTDATE" < '2016-12-01'
           |""".stripMargin.trim)
    }
